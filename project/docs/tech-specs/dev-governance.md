...
### 12.0.7 禁止静默降级实现（铁律 #21）

> 来源：v1.12

开发计划明确要求的技术方案（embedding 模型、算法、API 字段等），实现时必须按计划执行。如果无法实现，必须立即停止并报告，由用户决定是否接受降级。

常见降级行为（禁止）：
1. 计划要求 CLIP 视觉搜索，实现时偷换为关键词搜索
2. 计划要求 embedding 缓存，实现时跳过缓存直接调用 API
3. 计划要求返回 5 个 API 字段，实现时只返回 3 个
4. 计划要求 TDD 先写测试，实现时先写代码后补测试

### 12.0.8 Phase 4 逐条验收（铁律 #22）

> 来源：v1.12

Phase 4 审计中，禁止模糊声称"基本完成"。每条验收标准必须有明确 ✅/❌ 结论。Phase 4 Part D 第 8 项增加：技术方案降级检测——逐条对比计划指定的算法/策略数量、技术手段、API 字段与实际实现。

### 12.0.9 零代码前置检查

> 来源：v1.16

进入 Phase 1 时，先执行零代码前置检查。以下项目可在不写任何代码的情况下验证/修复：
- DB migration 是否已执行
- 环境变量是否已配置
- 依赖版本是否满足要求
- 外部服务连通性
...
如果问题已通过零代码手段解决，可跳过 Phase 2-5。

### 12.0.10 版本边界强制检测

> 来源：v1.16

Phase 7 完成后，必须读取开发计划逐条检查任务状态，输出格式化检测结果。禁止凭记忆判断版本是否完成。

版本边界处理禁止规则：
1. 禁止直接创建 PR（必须先完成版本收尾流程）
2. 禁止输出"完成"就停（必须执行完整的版本边界流程）
3. 禁止跳过任何步骤
4. 禁止在版本边界检测前声称版本已完成

### 12.0.11 Phase 4 到 Phase 4.5 强制过渡

> 来源：v1.17

Phase 4 门禁通过后，必须输出格式化确认（门禁状态 + 验收标准数量），然后进入 Phase 4.5。禁止从 Phase 4 直接跳到 Phase 5。

### 12.0.12 版本号识别规则

> 来源：v1.17

版本号以 VERSION 文件 + 开发计划文档为准。禁止仅靠 git log 推断版本号。

Smart Start 步骤 1.5.1 必须：
1. 读取 VERSION 文件获取当前版本号
2. 读取 TODO_NEXT.md 获取任务状态
3. 读取开发计划文档扫描任务全貌
4. 扫描其他分支/worktree 防止重复开发
