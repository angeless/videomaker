...
> 本节定义版本开发任务计划的写作标准和质量要求。AI 开发 Agent 不负责创建开发计划，但产品负责人编写的计划必须满足以下规范。Agent 在 Phase 1 门禁中应对照本节检查计划质量，如不符合则向用户反馈。

#### 3.5.1 写作原则

1. **以现有代码为准，不以文档为准** — 如果文档说有某个能力但代码里没有，以代码为准
2. **不写代码实现** — 定义做什么、输入输出是什么、规则是什么，但不写具体的函数体
3. **不自行扩张需求** — PRD 里没有的不加。如有建议的衍生功能，单独列在"建议但不在本版本范围内"段落
4. **不与已有代码冲突** — 新增模块独立目录，不改无关模块
5. **每个任务必须是最小可实现的闭环** — 做完这一个任务后系统可运行、可测试、可验收
6. **引用项目已有规范，不另起炉灶** — 版本号、命名、测试、文档产出等规则引用项目技术规范
7. **功能必须穷尽拆解为任务，一个都不能漏** — 版本目标中的每个功能点必须被拆解为具体的任务（vX.Y.Z），拆解后的任务列表必须**完整覆盖**该功能的所有实现工作。禁止出现"功能 X 的剩余部分后续处理"这种模糊遗留
8. **所有任务必须可验证** — 每个任务必须附带可执行的验收标准（输入什么 → 期望什么），不允许存在无法验证的任务
9. **禁止跨版本迁移任务** — 当前版本计划中定义的任务必须在当前版本内全部完成，不允许在开发过程中将任务推迟到下一个版本
...
| 功能拆解不完整 | 逐个功能点自查：执行完所有任务后此功能是否 100% 可用 |
| 任务不可验证（"完善 X"、"优化体验"） | 每个任务必须有输入→期望的验收 checklist |
| 将当前版本任务推迟到下个版本 | 当前版本定义的任务必须在当前版本完成，无法完成则停止反馈 |

#### 3.5.3 Agent Phase 1 门禁中的计划质量检查
...
| 功能拆解不完整 | 某功能执行完所有任务后仍无法 100% 使用 | 反馈给用户，请求补充遗漏的任务 |
| 存在不可验证的任务 | 任务目标含"完善"、"优化"但无具体验收指标 | 反馈给用户，请求定义可测试的验收标准 |
| 任务版本号不符合 vX.Y.Z 格式 | 使用了非标准编号 | 反馈给用户，请求按版本号规范重新编号 |
...
步骤 6:   轻量合规快检（五文件存在性、ci_verify.sh 可执行、VERSION 与 CHANGELOG 一致）
步骤 7:   输出检测结果摘要（含 📦 规范版本 行，不超过 10 行）
步骤 8:   进入自动连续开发模式
```
...
> **⚠️ 关键原则：分支上的所有操作（commit、push）无需等待用户确认。只有合并到 main 分支时才需要用户评审。**

**自动继续条件（全部满足）：**
1. Phase 4 审计门禁为 ✅ 或 ⚠️
2. Phase 6 汇报第 9 项为"建议继续"
3. 未触发任何停止条件（第 4 章停止条件）
...
> 本章定义质量工程实践，嵌入 Phase 1-8 流程中强制执行。

### 13.1 TDD — 测试驱动开发（嵌入 Phase 2）

Phase 2 编码实现**必须**遵循 Red-Green-Refactor 循环：
1. **Red**：先写失败的测试
2. **Green**：写最少代码让测试通过
3. **Refactor**：绿灯下重构

**例外**：纯配置/文档/依赖版本升级可免 TDD。

### 13.2 系统化调试（嵌入门禁失败流程）

门禁失败时**禁止简单重试**，必须 4 阶段根因调查：

| 阶段 | 操作 |
|------|------|
| D1 根因定位 | 阅读完整错误信息、堆栈、日志 |
| D2 模式识别 | 对照 common-errors.md 和 bug-catalog |
| D3 假设验证 | 最多 3 个根因假设，逐一验证 |
| D4 修复实施 | 针对确认根因修复 |

- 最多 3 次修复，3 次失败 → 停止等待用户

### 13.3 代码审查（嵌入 Phase 4 后）

Phase 4 后可派独立子 Agent 审查。反馈分级：Critical 阻断 / Important 必修 / Minor → WISHLIST。

### 13.4 并行子 Agent（嵌入 Phase 2）

3+ 独立问题可并行派发。每个 Agent 聚焦单一模块，无共享状态，相同文件禁止并行编辑。

### 13.5 完成前验证（嵌入 Phase 6 门禁）

**禁止无证据声称完成。** 5 步：列出断言 → 运行命令 → 阅读输出 → 标注 ✅/❌ → 全 ✅ 才可声明。

### 13.6 报告归档与编辑历史

规范文件更新用 Edit 增量，禁止 Write 覆盖。审计报告禁止删除原始发现。
...
计划要求的技术方案必须按计划执行。禁止：embedding→keyword / 3策略→2策略 / 省略API字段 / 简化算法。
无法实现 → 停止报告，用户决定。

### 铁律 #22：Phase 4 逐条验收

禁止模糊基本完成。每条验收标准：✅ 已实现（附证据）/ ❌ 未实现（附原因）。

---

## 15. 版本边界强制检测（v1.16+）

Phase 7 后必须读开发计划逐条检查任务状态，输出：

❌ 不读计划就声称完成 / ❌ 跳过空闲循环 / ❌ 跳过 Phase 8 / ❌ 直接创建 PR

---

## 16. 零代码前置检查（v1.16+）

Phase 1 前检查：DB migration / 环境变量 / 依赖 / 外部连通性 / 缓存。无需代码修复的问题直接跳到 Phase 6。

---

## 17. Phase 4→4.5 强制过渡（v1.17+）

Phase 4 通过后必须输出确认才能进入 4.5：

禁止 Phase 4 直接跳 Phase 5。

---

## 18. 版本号识别（v1.17+）

以 VERSION 文件 + 开发计划为准。❌ 禁止仅靠 git log / ❌ 禁止跳过开发计划 / ❌ 禁止忽略其他分支。

---

## 19. CI/Pre-commit/安全中间件（v1.18+）

### 19.1 CI Workflow 自动生成
已有 CI 不覆盖。无 CI 按 remote 平台生成（GitHub/GitLab/Bitbucket）。

### 19.2 Pre-commit Hook
commit 前自动运行 lint + typecheck + 占位符扫描。失败阻断。main 分支禁止直接 commit。

### 19.3 安全中间件测试隔离（BP-012）
测试中禁用/mock CSRF/rate-limit 等中间件。CI 环境变量 TESTING=1。中间件任务验收须含"现有测试通过"。
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v1.0 | 2026-03-19 | 初版七阶段流程 |
| v2.0 | 2026-03-20 | 第 12 章自动化基础设施 |
| v2.1 | 2026-03-22 | 升级 v1.4：Phase 4.5、§12.9、§3.5、product-standards |
| v3.0 | 2026-03-28 | 升级 v1.18：§13 质量工程、§14 铁律#21/#22、§15 版本边界、§16 零代码、§17 Phase 4→4.5 过渡、§18 版本号、§19 CI/Pre-commit/BP-012 |
