...
class FFmpegRenderer:
    ...
    def render_video(
        ...
        try:
            ...
            raise RuntimeError("FFmpeg 渲染超时（>3600s），已终止")
    ...
    def concat_videos(self, video_list: List[str], output_video: str) -> str:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError("视频合并超时（>3600s），已终止")
...
class VideoPipeline:
            ...
            if has_face and self.renderer.config.enable_skin_smooth and HAS_ADVANCED_BEAUTY:
                try:
                    ...
                    clip_cmd.extend(["-r", "30", "-vsync", "cfr",
                                     ...
                                     "-c:a", "aac", clipped])
                    subprocess.run(clip_cmd, capture_output=True, check=True, timeout=3600)
                    ...
                    merge_cmd = [self.renderer.ffmpeg_path, "-y",
                                 ...
                                 "-c:v", "copy", "-c:a", "copy",
                                 beauty_output]
                    subprocess.run(merge_cmd, capture_output=True, check=True, timeout=3600)
                    print(f"   磨皮: 频率分解（高级）")
    ...
    def _mix_bgm(self, video_path: str, bgm_path: str, output_path: str) -> str:
        ...
        try:
            ...
            print(f"  ⚠️  BGM 混合超时（>3600s），保留原音频")
            shutil.copy(video_path, output_path)
            return output_path
        if result.returncode != 0:
