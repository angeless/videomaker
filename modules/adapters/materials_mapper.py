...
def materials_to_search_index(materials: dict) -> dict:
    """将 workflow `materials.json` 转为 `VideoSearch` 可消费结构。"""
    converted = {}
    for vid_id, vdata in (materials or {}).items():
        filename = vdata.get("filename", vid_id)
        metadata = vdata.get("analysis", {}).get("metadata", {})
        local = vdata.get("analysis", {}).get("local_analysis", {})
        objects = local.get("objects", {}).get("detected_objects", [])
        ...
        converted[vid_id] = {
            ...
            "index_data": {
                "tags": list(objects),
                "search_keywords": [desc, mood, filename],
                "preview_info": {
            ...
            "content_summary": {"notes": [desc]},
            "technical_summary": {"resolution": resolution, "duration": duration},
        }
