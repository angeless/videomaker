...
class AdaptiveRewriter:
    ...
    # 词汇替换规则（来自 opencut/script/adaptive.py + 本地扩展）
    _SUBSTITUTIONS = {
        "奔跑": "漫步", "跑": "走", "追逐": "跟随", "跳跃": "站立",
        "攀登": "欣赏", "冲浪": "漫步海边", "滑翔": "俯瞰",
    }

    def _apply_substitutions(self, text: str) -> str:
        """应用词汇替换规则（素材不足时降低动态描述）"""
        result = text
    ...
    def _adjust_text_to_material(
        ...
        # 先应用词汇替换规则
        original_text = self._apply_substitutions(original_text)
...
class ScriptGapAnalyzer:
    ...
    def analyze_coverage(
        ...
        for i, (clip, subtitle) in enumerate(itertools.zip_longest(
            script.get("clips", []),
